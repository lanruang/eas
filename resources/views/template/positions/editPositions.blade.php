...
@section('content')
	<div class="row">
		<div class="col-xs-12">
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" id="validation-form" method="post" action="{{route('positions.updatePositions')}}" >
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right"> 上级岗位 </label>
					<label class="col-sm-2 output" id="pos_pid_list">{{ $pos['p_name'] }}</label>
					<input type="hidden" name="pos_pid" id="pos_pid" value="{{ $pos['pid'] }}"/>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" name="pos_name" id="pos_name" placeholder="岗位名称" class="form-control" value="{{ $pos['name'] }}" />
					</div>
				</div>
