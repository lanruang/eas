...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            ...
            <!-- PAGE CONTENT BEGINS -->
            <form class="form-horizontal" role="form" id="validation-form" method="post" action="{{ route('contract.updateContract') }}" >
                            ...
                            <select class="form-control" id="contract_class" name="contract_class">
                                ...
                                @foreach ($select as $v)
                                    @if ($v['ass_type'] == 'contract_class')
                                        <option value="{{ $v['ass_value'] }}" @if ($v['ass_value'] == $contract['cont_class']) selected @endif>{{ $v['ass_text'] }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </label>
                            ...
                            <select class="form-control" id="contract_type" name="contract_type">
                                ...
                                @foreach ($select as $v)
                                    @if ($v['ass_type'] == 'contract_type')
                                        <option value="{{ $v['ass_value'] }}" @if ($v['ass_value'] == $contract['cont_type']) selected @endif>{{ $v['ass_text'] }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </label>
                ...
                <div id="contractPartiesFarm" class="form-group hide">
                    ...
                    <div class="col-sm-5">
                        <label class="control-label align-left" id="text_parties">{{ $contract['parties_name'] }}</label>
                        <input type="hidden" id="contract_parties" name="contract_parties" value="{{ $contract['cont_parties'] }}"/>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <input type="text" name="contract_num" id="contract_num" placeholder="合同编号" class="form-control" value="{{ $contract['cont_num'] }}" />
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <input type="text" name="contract_name" id="contract_name" placeholder="合同名称" class="form-control" value="{{ $contract['cont_name'] }}" />
                    </div>
                </div>
                ...
                <div class="form-group" id="budgetDateFarm">
                    ...
                    <div class="col-sm-3">
                        <div class="input-group">
                            ...
                            <input class="form-control" type="text" name="contract_date" id="contract_date" value="{{ $contract['cont_start'] }} 一 {{ $contract['cont_end'] }}" />
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group" id="budgetDateFarm">
                    ...
                    <div class="col-sm-2">
                        <input class="form-control text-right" type="text" name="contract_amount" placeholder="0.00" id="contract_amount" onblur="formatAmount('contract_amount')" value="{{ $contract['cont_sum_amount'] }}"/>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <label class="control-label align-left" id="subject_text">{{ $contract['sub_ip'] }}<br>{{ $contract['sub_name'] }}</label>
                        <input type="hidden" id="contract_subject" name="contract_subject" value="{{ $contract['cont_subject'] }}"/>
                    </div>
                ...
                <div class="form-group" id="budgetDateFarm">
                    ...
                    <div class="col-sm-3">
                        <textarea class="input-xlarge" name="contract_remark" id="contract_remark">{{ $contract['cont_remark'] }}</textarea>
                    </div>
                </div>
                            ...
                            <div class="widget-body">
                                <table class="table" style="margin-bottom: 0;">
                                    ...
                                    @foreach ($contDetails as $v)
                                        <tr date="{{ $v['cont_details_date'] }}" amount="{{ toDecimal($v['cont_amount']) }}">
                                            <td class="align-middle center">{{ $v['cont_details_date'] }}</td>
                                            <td class="align-middle align-right">{{ toDecimal($v['cont_amount']) }}</td>
                                            <td class="align-middle center">
                                                <button type="button" class="btn btn-danger btn-xs" onclick="delIncomePayPeriod('{{ $v['cont_details_date'] }}', '{{ toDecimal($v['cont_amount']) }}    ', '{{ $v['details_id'] }}')">
...
@section('FooterJs')
    <script type="text/javascript">
        var partiesUrl;
        var partiesType = '{{ $contract['cont_class'] }}';
        var partiesTable;
        var sumAmount = 0;
        var budgetTable;
        var budgetId = '';
        ...
        $(function($) {
            $("#contractPeriod tr").each(function() {
                sumAmount += parseFloat($(this).attr('amount'));
            });
        ...
        function addIncomePayPeriod(){
            ...
            var trHtml = '<tr date='+date+' id=""><td class="align-middle center">'+date+'</td>' +
        ...
        //删除收付期间
        function delIncomePayPeriod(date, amount, id){
            var dates = $('#contract_dates').val();
            var isEdit = false;
            dates = dates.split("|");
            for(i in dates){
        ...
        function delDetails(id){
            ...
            var result = ajaxPost(data, '{{ route('contract.delDetails') }}');
            return result;
        }
